...
def create_graph(dict_sum, household_cost):
    x = ['Water Usage Sum', 'Household Cost']
    y = [dict_sum, household_cost]
    
    plt.plot(x, y)
    plt.title('Water Usage vs. Household Cost')
    plt.xlabel('Category')
    plt.ylabel('Value')
    plt.show()
def show_graph():
    ...
    dict_sum, household_cost = on_calculate()

    create_graph(household_cost, dict_sum)
